...
/*   Updated: 2024/10/11 17:33:54 by myakoven         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	redir_cmd(t_redircmd *rcmd)
{
	...
	if (rcmd->fd == -1)
	{
		...
		exit(errno);
		// exit(redir_error(rcmd)); // exit but not with one... errno???? yeah
	} // MYAKOVEN addded curly braces
	exec_cmd(rcmd->cmd);
}

void	errno_exit(t_tools *tools)
{
}
